				...
				<TablePadding :title="title" :list="list" :key="refresh" :ischeckbox="true" :isexcel="false" 
					...
					<div slot="btn_solt">
						<el-button type="success" size="mini" icon="el-icon-circle-plus-outline"
							@click="tolink('JewelleryAdd')">添加</el-button>
					...
					<el-table-column prop="prompt" label-class-name="prompt" label="提示语"  />
					<el-table-column label="价格" >
						<template slot-scope="scope">
							<el-input-number size="mini" v-model="scope.row.pjine" :min="0"  :precision="2"
					...
					<!-- <el-table-column label="上架费比例" >
						<template slot-scope="scope">
							<el-input-number size="mini" v-model="scope.row.sjbili" :min="0" :precision="2" @change="Update(scope.row)"
								:controls="false"></el-input-number>%
						</template>
					</el-table-column> -->
					<!-- <el-table-column label="收益比例" >
						<template slot-scope="scope">
							<el-input-number size="mini" v-model="scope.row.sybili" :min="0" :precision="2" @change="Update(scope.row)"
								:controls="false"></el-input-number>%
						</template>
					</el-table-column> -->
					
					<!-- <el-table-column label="显示前台" width="80">
						<template slot-scope="scope">
					...
					<el-table-column label="操作">
						<template slot-scope="scope">
							<router-link :to="'JewelleryUpdate?Id=' + scope.row.id" class="routerlink">编辑</router-link>
						</template>
					</el-table-column>
				</TablePadding>
			</div>
