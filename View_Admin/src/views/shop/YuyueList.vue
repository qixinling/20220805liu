				...
				<TablePadding :title="title" :list="list" :key="refresh" :ischeckbox="true" :isexcel="false" 
					...
					<div slot="btn_solt">
						<el-button type="success" size="mini" icon="el-icon-circle-plus-outline"
							@click="Passyuyue">预约成功</el-button>
						<el-button icon="el-icon-delete" type="danger" size="mini" @click="Delete">删除</el-button>
					</div>
					<el-table-column prop="ydate" label-class-name="ydate" label="预约时间"  />
					...
					<el-table-column prop="cjindan" label="押金单"  />
					<el-table-column prop="czhouqi"  label="周期"  />
					<el-table-column prop="crishouri" label="日收益(%)"  />
					<el-table-column prop="cqdate" label="开抢时间"  />	
				</TablePadding>
			</div>
		</div>
	</div>
				...
				if (this.$refs.tablepadding.select.length == 0) {
					...
					this.$confirm("确定要预约成功吗?", "提示", {
						...
						.then(() => {
							...
							this.$request.post(
								"Api/UsersYuyue_Admin/Passyuyue",
								{
									token_admin: this.$utils.getloc("token_admin"),
									userid_admin: this.$utils.getloc("userid_admin"),
									ids:","+ ids.toString()+",",
