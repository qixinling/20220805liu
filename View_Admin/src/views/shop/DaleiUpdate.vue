<template>
	<div>
		<PageTitle title="大类修改" :btn="false" :excel="false"></PageTitle>
		<div class="cardbox">
					...
					<el-form-item label="图片">
						<el-upload class="avatar-uploader" :action="UploadFile" :show-file-list="false" :on-success="handleAvatarSuccess"
						 :before-upload="beforeAvatarUpload">
							<img v-if="imageUrl" :src="getimg(form.daleiimg)" class="avatar" />
			...
			getdata: function() {
				this.$request.post(
					...
					(res) => {
						...
						this.form.putaway = this.form.putaway == 0 ? false : true;
						this.form.pagemark = this.form.pagemark == 0 ? false : true;
						this.imageUrl = this.form.daleiimg;
					}
				);
			},
			Update: function() {
				this.$confirm('确定要修改吗?', '提示', {
					...
					this.$request.post(
						"Api/ShopGoodsSort_Admin/Update", {
							token_admin: this.$utils.getloc("token_admin"),
							userid_admin: this.$utils.getloc("userid_admin"),
							id: this.did,
