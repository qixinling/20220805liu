<template>
	<div>
		<PageTitle title="临时会员" ref="pageTitle"></PageTitle>
		<div class="cardbox">
			<div class="cardcontent">
				<TablePadding title="正式会员" :list="list" :key="refresh" :ischeckbox="true" :isexcel="true" :summary="false" ref="tablepadding">
					<!--操作按钮插槽-->
					<div slot="btn_solt">
						<el-button icon="el-icon-unlock" type="success" size="mini" @click="jihuo">激活</el-button>
					...
					<el-table-column label-class-name="pdt" label="激活时间" width="200">
						<template slot-scope="scope">{{ scope.row.pdt }}</template>
					</el-table-column>
					...
					<el-table-column prop="lsk" label-class-name="lsk" label="个人业绩" width="100"></el-table-column>
					...
					<el-table-column label="万能账号" width="200">
						<template slot-scope="scope" >
							<span v-if="scope.row.iswn == 0">否</span>
							<span v-if="scope.row.iswn == 1">是</span>
						</template>
					</el-table-column>
	...
	export default {
		...
		methods: {
			...
			jihuo: function() { 
				...
				_this.$confirm('确定要激活吗?', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).then(() => {
					_this.$request.post("Api/Users_Admin/Jihuo", {
						token_admin: _this.$utils.getloc('token_admin'),
						userid_admin: _this.$utils.getloc('userid_admin'),
						ids: "," +idlist.toString() + ","
					}, (res) => {
