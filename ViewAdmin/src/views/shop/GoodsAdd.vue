<template>
	<div style="margin-bottom: 200px">
		<PageTitle title="添加商品" :btn="false" :excel="false"></PageTitle>
				...
				<el-form ref="form" label-width="150px">
					...
					<el-form-item label="分类">
						<el-select v-model="goods.Xlid" placeholder="请选择分类">
							<el-option v-for="item in sort_child_list" :key="item.id" :label="item.xiaoleiname"
			...
			GetXiaolei: function () {
				...
				_this.$request.post(
					...
					(res) => {
						...
						if (_this.sort_child_list.length > 0) {
							_this.goods.Xlid = _this.sort_child_list[0].id;
						}
					}
				);
			},
			...
			Add() {
				...
				if (mod.pass) {
					_this.$confirm('确定要添加吗?', '提示', {
						...
						for (let i = 1; i <= num; i++) {
							nr = nr.replace(_this.$config.img_url, "");
						}
						_this.$request.post(
							"Api/ShopGoods_Admin/Add",
							{
								token_admin: _this.$utils.getloc("token_admin"),
								userid_admin: _this.$utils.getloc("userid_admin"),
