<template>
	<div class="newsupdate">
		<PageTitle title="修改新闻" :btn="false" :excel="false"></PageTitle>
		<div class="cardbox">
			<div class="cardcontent">
				<el-form ref="form" label-width="80px">
					...
					<el-form-item>
						<el-button type="success" size="mini" @click="updatenews" icon="el-icon-edit">修改</el-button>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getdata: function() {
				this.$request.post("Api/News_Admin/Get", {
					token_admin: this.$utils.getloc('token_admin'),
					userid_admin: this.$utils.getloc('userid_admin'),
					id: this.nid
				}, (res) => {
					this.title = res.data.data.news_title;
					this.display = res.data.data.display;
					this.mrdisplay = res.data.data.display;

					if (res.data.data.news_cover != "") {
						var img = {
							url: this.$config.img_url + res.data.data.news_cover
						}
						this.fileList.push(img);
					}

					let nr = res.data.data.news_content;
					nr = nr.replace(/src="/g, 'src="' + this.$config.img_url); // 把字符串里面所有的src="替换成后面的字符
					this.content = nr;
				});
			},
			...
			updatenews: function() {
				...
				this.$request.post("Api/News_Admin/Update", {
