<template>
    ...
    <div class="cardBox">
      <div style="margin-bottom:10px" class="cardContent" v-for="(item,index) in data" :key="index">
		  <van-row style="padding: 5px;font-size: 13px;">
			...
			<van-col span="6" style="text-align: right;color: #FC4702;" >{{item.statename}}</van-col>
		  </van-row>
        <van-row>
          <van-col span="8">
  ...
  <van-dialog v-model="showBank" show-cancel-button :confirmButtonText="showBankBtnLx == 1 ? '确认付款' : '确认申请'" @confirm="confirmZhongzhuan">
    <div class="dialog-bank">
        <div style="padding:20px 10px" v-if="orderActiveObj.state != 3">
          <div class="bank-title">卖家账户</div>
          <div style="border-bottom: 1px solid #E6A23C;margin-bottom:10px" v-for="(bank,bankindex) in orderActiveObj.banklist" :key="bankindex">
        ...
        <div style="padding:20px 10px" v-if="orderActiveObj.state == 3">
          <div class="bank-title">画室长账户</div>
			    <div style="border-bottom: 1px solid #E6A23C;margin-bottom:10px" v-for="(bank2,bank2indx) in orderActiveObj.hsbanklist" :key="bank2indx">
			      <van-cell center title="账户名称" :value="bank2.bankname" />
			      <van-cell center title="账号" :value="bank2.bankcard">
          <template #right-icon>
            <img style="padding-left: 10px;" src="../../assets/img/fz.png" @click="CopyLink(bank2.bankcard)"/>
          </template>
        </van-cell>
              <van-cell center title="姓名" :value="bank2.username">
          <template #right-icon>
            <img style="padding-left: 10px;" src="../../assets/img/fz.png" @click="CopyLink(bank2.username)"/>
          </template>
        </van-cell>
              <van-cell center title="电话" :value="bank2.usertel">
          <template #right-icon>
            <img style="padding-left: 10px;" src="../../assets/img/fz.png" @click="CopyLink(bank2.usertel)"/>
          </template>
        </van-cell>
          <div style="text-align:center">
