        ...
        <div class="cardBox" v-for="(item, index) in list" :key="index" @click="tz(item.id)">
            <div class="allContent">
                <van-row>
                    ...
                    <van-col span="12" class="text" style="text-align: right">{{item.coinname}}</van-col>
                </van-row>
                <van-row style="padding-top: 12px">
                    <van-col span="5" style="font-size: 14px">{{ $utils.timestampFormat(item.zdate) }}</van-col>
                    <van-col span="19" style="text-align: right">
                        <div class="pricetext" style="">
                            {{ item.jine}}<span style="font-size: 8px; font-weight: 400">元</span>
                        </div>
                    </van-col>
                </van-row>
                <van-row style="padding-top: 8px">
                    <van-col span="12" class="text">付款人 {{ item.userid }}</van-col>
                    <van-col span="12" class="text" style="text-align: right">收款人 {{ item.suserid }}</van-col>
                </van-row>
...
export default {
    ...
    methods: {
        getdata: function () {
            ...
            _this.$request.post(
                "api/WalletsZhuanzhang/ZhuanzhangGet",
                {
